...
/*                                                                    */
/*  Canonical CLI — former repo root `src/cli/*` shims removed (PR-4).   */
/* ------------------------------------------------------------------ */

import { TraceWriter } from '@ai-video/pipeline-core/traceWriter.js';
import {
  buildTimeline,
  findFailureSpan,
  buildProviderDecisionPath,
  buildStageDiff,
} from '@ai-video/pipeline-core/traceAnalyzer.js';
import {
  formatSummary,
  formatTimeline,
  formatFailureSpan,
  formatProviderPath,
  formatStageDiff,
} from '@ai-video/pipeline-core/traceFormatter.js';
...
function printUsage(): void {
  console.log(`
Usage: npx tsx apps/server/src/cli/replay.ts <arg> [options]
...
Examples:
  npx tsx apps/server/src/cli/replay.ts ./data/projects/my-project/trace/trace-abc123.json
  npx tsx apps/server/src/cli/replay.ts my-project
  npx tsx apps/server/src/cli/replay.ts abc123def456...  (32-char hex)
  npx tsx apps/server/src/cli/replay.ts my-project --json
  npx tsx apps/server/src/cli/replay.ts my-project --format failure
  npx tsx apps/server/src/cli/replay.ts --list
`);
}
