...
export async function parseMultipartFile(req: IncomingMessage, maxSize: number): Promise<MultipartFile> {
  ...
  if (boundary == null || boundary === '') throw new Error('Missing multipart boundary');
  ...
  for (const part of parts) {
    ...
    const penultimate = body.length - 2;
    if (body.length >= 2 && body[penultimate] === 0x0d && body[last] === 0x0a) {
      body = body.subarray(0, penultimate);
    }
