...
import type { WorkbenchEvent } from '@ai-video/shared/types.js';
import { WB_EVENT } from '@ai-video/shared/types.js';
import { createLogger } from '@ai-video/lib/logger.js';

const log = createLogger('BGMLibrary');
...
function getAudioDuration(filePath: string): number | null {
  try {
    ...
    const parsed = JSON.parse(res.stdout) as { format?: { duration?: string | number } };
    const dur = typeof parsed.format?.duration === 'string'
