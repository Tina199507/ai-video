...
/* ---- Pin app name so user-data path stays stable across the
 * direction A-3 rename (legacy: ai-video-browser-shell, new package
 * id: @ai-video/app-desktop).  Without this, Electron derives
 * userData from package.json `name` and existing dev installations
 * would lose access to ~/Library/Application Support/ai-video-browser-shell.
 */
app.setName('ai-video-browser-shell');

/* ---- Load .env from project root (if present) ---- */
try {
  // app.getAppPath() → apps/desktop/, two levels up = monorepo root where .env lives
...
function resolveAppUrl(): string {
  ...
  // where apps/ui-shell/dist lives (post direction A-2 physical move).
  const projectRoot = path.resolve(app.getAppPath(), '..', '..');
  const uiDistIndexPath = path.join(projectRoot, 'apps', 'ui-shell', 'dist', 'index.html');
...
const bootCallbacks: BootCallbacks = {
  ...
  resolveBackend: () => {
    // Sidecar integrity gate — fail fast if the packaged binary or the
    // bundled chromium folder doesn't match the manifest written at
    // build time (tampering / partial install / mixed artifacts).
    // `strict: false` keeps older builds without a manifest loadable.
    const verification = verifySidecarIntegrity({ strict: false });
    if (!verification.ok) {
      const msg =
        `Sidecar integrity check failed: ${verification.reason}` +
        (verification.detail ? ` (${verification.detail})` : '');
      console.error('[Main]', msg);
      throw new Error(msg);
    } else if (verification.reason === 'verified' && verification.manifest) {
      console.log(
        `[Main] Sidecar verified — gitSha=${verification.manifest.gitSha.slice(0, 12)} ` +
        `builtAt=${verification.manifest.builtAt}`,
      );
    }
