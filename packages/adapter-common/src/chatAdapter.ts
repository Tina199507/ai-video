export * from '@ai-video/pipeline-core/chatAdapter.js';
/* ------------------------------------------------------------------ */
...
import type { ProviderId } from '@ai-video/shared/types.js';
import type { ChatWorkbenchPort } from './chatAdapter.port.js';
import { copyFileSync, existsSync, mkdirSync, readdirSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';
import { extractJSON, isTruncated, mergeContinuation } from './responseParser.js';
import { getAdapterHostBindings, type HostTtsConfig } from './hostBindings.js';
...
export class ChatAdapter implements AIAdapter {
  ...
  private workbench: ChatWorkbenchPort;
  private config: Required<ChatAdapterConfig>;
  private readonly host = getAdapterHostBindings();
  private pollinationsUnavailable = false;
  private pollinationsFailureCount = 0;

  constructor(workbench: ChatWorkbenchPort, config?: Partial<ChatAdapterConfig>) {
    this.workbench = workbench;
    const h = this.host;
    this.config = {
      assetsDir: config?.assetsDir ?? 'data/projects/assets',
      defaultTextProvider: (config?.defaultTextProvider ?? h.defaultTextProvider) as ProviderId,
      imageProvider: (config?.imageProvider ?? h.defaultImageProvider) as ProviderId,
      defaultModel: config?.defaultModel ?? '',
      responseTimeoutMs: config?.responseTimeoutMs ?? h.timeouts.chatResponseTimeoutMs,
      maxContinuations: config?.maxContinuations ?? h.timeouts.maxContinuations,
      sessionId: config?.sessionId ?? '',
      continueChat: config?.continueChat ?? false,
    };

    h.quota.on((event) => {
      if (event.exhausted && event.capability === 'image' && event.accountId) {
        h.capabilityQuota.markExhausted(event.accountId, 'image', event.provider);
      }
    });
  }
  ...
  async generateImage(
    ...
    const providerResources = this.config.imageProvider
      ...
      : imageResources;
    if (this.host.capabilityQuota.allExhausted(providerResources, 'image')) {
    ...
    if (!answer.trim() || this.host.hasQuotaSignal(answer)) {
      // Mark THIS specific account, not a global flag
      const accountId = this.workbench.getActiveAccountId();
      if (accountId) {
        this.host.capabilityQuota.markExhausted(accountId, 'image', String(this.config.imageProvider));
      }
      this.host.quota.emit({
    ...
    if (page) {
      ...
      if (selectors) {
        ...
        const extracted = await this.host.extractLatestImage(
          page,
  ...
  private async generatePollinationsImage(
    ...
    const tryFetch = async (proxyUrl?: string): Promise<Response> => {
      ...
      const timer = setTimeout(() => controller.abort(), this.host.timeouts.pollinationsFetchTimeoutMs);
      try {
        if (!proxyUrl) {
    ...
    try {
      ...
      const maxAttempts = this.host.timeouts.pollinationsMaxAttempts;
      for (let attempt = 1; attempt <= maxAttempts && !resp; attempt++) {
        let sawRateLimit = false;
        const proxyTargets: Array<string | undefined> = this.host.httpProxy
          ? [undefined, this.host.httpProxy]
          : [undefined];
  ...
  async generateSpeech(
    ...
    try {
      return await this.host.tts.generateSpeech(text, ttsConfig);
    } catch (err) {
