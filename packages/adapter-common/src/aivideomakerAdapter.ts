export * from '@ai-video/pipeline-core/aivideomakerAdapter.js';
/* ------------------------------------------------------------------ */
...
import { withRetry as sharedWithRetry } from '@ai-video/lib/retry.js';
...
/** AIVideoMaker-scoped withRetry — thin wrapper over the shared helper. */
function withRetry<T>(fn: () => Promise<T>, options?: AIRequestOptions): Promise<T> {
  return sharedWithRetry(fn, { ...options, label: 'AIVideoMaker API request', logger: log });
}
