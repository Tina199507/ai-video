 ...
 */
function cycleVariant(sceneIndex: number): KenBurnsVariant {
  return CYCLE_VARIANTS[sceneIndex % CYCLE_VARIANTS.length] ?? 'center_zoom';
}

export function resolveKenBurnsVariant(cameraMotion: string | undefined, sceneIndex: number): KenBurnsVariant {
  if (!cameraMotion) return cycleVariant(sceneIndex);
  ...
  return cycleVariant(sceneIndex);
}
