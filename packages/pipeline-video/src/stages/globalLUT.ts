...
export function parseColorStats(ffmpegOutput: string): ColorStats | undefined {
  ...
  const yVals = [...ffmpegOutput.matchAll(yRegex)].map(m => parseFloat(m[1] ?? '0'));
  const uVals = [...ffmpegOutput.matchAll(uRegex)].map(m => parseFloat(m[1] ?? '0'));
  const vVals = [...ffmpegOutput.matchAll(vRegex)].map(m => parseFloat(m[1] ?? '0'));
  const satVals = [...ffmpegOutput.matchAll(satRegex)].map(m => parseFloat(m[1] ?? '0'));
