...
describe('chatAutomation – module structure', () => {
  ...
  it('shim re-exports the same bindings as @ai-video/pipeline-core', async () => {
    const mod = await import('./chatAutomation.js');
    const core = await import('@ai-video/pipeline-core/chatAutomation.js');
    // Port-layer wrappers may use `(...args) =>` and report `.length === 0`; compare identity instead.
    expect(mod.openChat).toBe(core.openChat);
    expect(mod.sendPrompt).toBe(core.sendPrompt);
  });
});
