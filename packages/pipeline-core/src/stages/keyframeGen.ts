...
export async function runKeyframeGen(
  ...
  for (let i = 0; i < results.length; i++) {
    const r = results[i];
    if (!r) continue;
    const irScene = getVideoIRScene(videoIR, r.number, i);
    r.assetType = irScene.assetType;
  }
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (!scene || scene.assetType !== 'video') continue;
    ...
    if (!scene.keyframeUrl && (scene.referenceImageUrl || scene.assetUrl)) {
      scene.keyframeUrl = scene.referenceImageUrl ?? scene.assetUrl;
    }
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (result.imageUrl) {
          scene.keyframeUrl = result.imageUrl;
        ...
        scene.logs.push(`Keyframe attempt ${attempt + 1} error: ${msg}`);
