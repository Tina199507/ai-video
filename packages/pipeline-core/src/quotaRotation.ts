...
import type { AiResource, ProviderId } from './sharedTypes.js';
import { createLogger } from '@ai-video/pipeline-core/libFacade.js';

const log = createLogger('QuotaRotation');
...
export class CapabilityQuotaTracker {
  ...
  markExhausted(resourceId: string, capability: QuotaCapability, provider?: string): void {
    ...
    log.info('exhausted', { resourceId, capability, provider });
  ...
  isExhausted(resourceId: string, capability: QuotaCapability): boolean {
    ...
    if (this.resetWindowMs > 0 && Date.now() - at >= this.resetWindowMs) {
      ...
      log.info('auto_reset', {
        resourceId,
        capability,
        hours: Math.round((Date.now() - at) / 3600000),
      });
      return false;
    }
    return true;
