...
import type { AiResource, AiResourceType, ProviderId, Account } from './runtimeTypes.js';
import { BUILTIN_PROVIDER_IDS, BUILTIN_PROVIDER_LABELS } from './providers.js';
import { writeJsonAtomic } from '@ai-video/pipeline-core/libFacade.js';
...
export class ResourceManager {
  ...
  constructor(savePath?: string, skipSeed?: boolean, options: { kv?: ProjectKVStore; key?: string } = {}) {
    this.savePath = savePath ?? join(process.cwd(), 'data', 'resources.json');
    if (options.kv) {
      this.kv = options.kv;
      this.kvKey = options.key ?? defaultKey(this.savePath, 'resources');
    } else {
      this.kv = new JsonKVStore(dirname(this.savePath));
      this.kvKey = defaultKey(this.savePath, 'resources');
    }
  ...
  private load(): void {
    let loaded: AiResource[] | null | undefined;
    try {
      loaded = this.kv.get<AiResource[]>(this.kvKey) ?? null;
    } catch (err) {
      log.warn('resources kv corrupt, attempting migration', { err: String(err), path: this.savePath });
      loaded = null;
    }
  ...
  private tryMigrate(): boolean {
    ...
    if (existsSync(accountsPath)) {
      try {
        ...
        migrated = true;
        log.info('migrated accounts.json', { count: accounts.length });
      } catch (err) {
        log.warn('failed to migrate accounts.json', { err: String(err) });
      }
    }
    ...
    if (existsSync(configPath)) {
      try {
        ...
        if (vpc?.url) {
          ...
          migrated = true;
          log.info('migrated video provider from config.json');
        }
      } catch (err) {
        log.warn('failed to migrate config.json videoProviderConfig', { err: String(err) });
      }
    }
  ...
  private persist(): void {
    this.kv.set(this.kvKey, this.resources);
  }
  ...
  checkAndResetQuotas(): number {
    ...
    for (const r of this.resources) {
      ...
      if (exhaustedAt > 0 && now - exhaustedAt >= this.resetWindowMs) {
        ...
        resetCount++;
        log.info('auto-reset quota', {
          label: r.label,
          provider: r.provider,
          exhaustedHoursAgo: Math.round((now - exhaustedAt) / 3600000),
        });
      }
    }
