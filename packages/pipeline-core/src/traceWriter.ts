// @ts-nocheck -- see tsconfig.json noUncheckedIndexedAccess migration
...
import type { AnyTraceEvent, TraceReplayBundle, StageSummary, ReplayTotals, FailureDescriptor } from './traceEvents.js';
...
export class TraceWriter {
  ...
  constructor(private readonly traceId: string, private readonly projectId: string, projectDir: string) {
  ...
  buildReplayBundle(meta: TraceWriterMeta): TraceReplayBundle {
    ...
    const totals: ReplayTotals = { stagesCompleted: 0, stagesFailed: 0, llmCalls: 0, costUsd: 0, retries: 0 };
    ...
    for (const evt of this.events) {
      switch (evt.kind) {
        case 'stage.start':
          stageSummary[evt.data.stage] = { status: 'not_started', retries: 0 };
          break;
        case 'stage.complete':
        ...
        case 'stage.error':
          ...
          break;
        case 'stage.retry':
          if (stageSummary[evt.data.stage]) stageSummary[evt.data.stage].retries++;
          totals.retries++;
          break;
        case 'stage.skip':
          stageSummary[evt.data.stage] = { status: 'skipped', retries: 0 };
          break;
        case 'ai_call.start':
          totals.llmCalls++;
          break;
