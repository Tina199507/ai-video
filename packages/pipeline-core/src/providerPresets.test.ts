...
import type { SiteAutomationConfig } from './runtimeTypes.js';
...
describe('providerPresets', () => {
  ...
  describe('site automation preset overrides', () => {
    const originalEnv = { ...process.env };
    let tempDir: string;

    beforeEach(() => {
      tempDir = mkdtempSync(join(tmpdir(), 'preset-overrides-'));
      process.env.DATA_DIR = tempDir;
      delete process.env.APPDATA_DIR;
    });
    ...
    it('falls back to bundled presets when DATA_DIR has none', async () => {
      vi.resetModules();
      const mod = await import('./providerPresets.js');
      mod.invalidatePresetCache();
      const ids = Object.keys(mod.PROVIDER_PRESETS);
      expect(ids).toContain('chatgpt');
      expect(ids).toContain('gemini');
      expect(mod.PROVIDER_PRESETS.chatgpt.label).toBe('ChatGPT');
    });

    it('merges partial overrides field-by-field over bundled defaults', async () => {
      writeFileSync(join(tempDir, 'site-automation-overrides.json'), JSON.stringify({
        chatgpt: {
          label: 'My ChatGPT',
          timing: { maxWaitMs: 10_000 },
          dailyLimits: { text: 999 },
        },
      }));
      ...
      const mod = await import('./providerPresets.js');
      mod.invalidatePresetCache();
      const preset = mod.PROVIDER_PRESETS.chatgpt;
      expect(preset.label).toBe('My ChatGPT');
      expect(preset.timing.maxWaitMs).toBe(10_000);
      expect(preset.timing.pollIntervalMs).toBe(2_000);
      expect(preset.dailyLimits?.text).toBe(999);
      expect(preset.selectors.promptInput.length).toBeGreaterThan(0);
    });

    it('allows introducing a brand-new provider via overrides', async () => {
      writeFileSync(join(tempDir, 'site-automation-overrides.json'), JSON.stringify({
        customsite: {
          id: 'customsite',
          label: 'Custom Site',
          ...
          selectors: {
            ...
            generateButton: [{ selector: 'button', method: 'css', priority: 1 }],
            resultElement: [{ selector: 'div', method: 'css', priority: 1 }],
          },
          timing: { maxWaitMs: 1000, pollIntervalMs: 500, hydrationDelayMs: 100 },
          profileDir: '',
        },
      }));
      ...
      const mod = await import('./providerPresets.js');
      mod.invalidatePresetCache();
      expect(mod.PROVIDER_PRESETS.customsite).toBeDefined();
      expect(mod.PROVIDER_PRESETS.customsite.label).toBe('Custom Site');
    });

    it('savePresetOverride persists edits and invalidates the cache', async () => {
      vi.resetModules();
      const mod = await import('./providerPresets.js');
      mod.invalidatePresetCache();
      expect(mod.PROVIDER_PRESETS.chatgpt.label).toBe('ChatGPT');

      mod.savePresetOverride('chatgpt', { label: 'Persisted' });
      expect(mod.PROVIDER_PRESETS.chatgpt.label).toBe('Persisted');

      const saved = JSON.parse(readFileSync(join(tempDir, 'site-automation-overrides.json'), 'utf-8'));
      expect(saved.chatgpt.label).toBe('Persisted');
    });

    it('ignores invalid override JSON and falls back to bundled defaults', async () => {
      writeFileSync(join(tempDir, 'site-automation-overrides.json'), '{not-json');
      vi.resetModules();
      const mod = await import('./providerPresets.js');
      mod.invalidatePresetCache();
      expect(mod.PROVIDER_PRESETS.chatgpt.label).toBe('ChatGPT');
    ...
    it('PROVIDER_PRESETS is a read-only proxy', async () => {
      vi.resetModules();
      const mod = await import('./providerPresets.js');
      mod.invalidatePresetCache();
      const before = mod.PROVIDER_PRESETS.chatgpt.label;
      expect(() => {
        // @ts-expect-error — intentionally mutating a read-only proxy
        mod.PROVIDER_PRESETS.chatgpt = { label: 'Hacked' };
      }).toThrow(TypeError);
      expect(mod.PROVIDER_PRESETS.chatgpt.label).toBe(before);
    });
  });
});
