...
const brLog = createLogger('BrowserManager');
...
export function resolveChromiumChannel(): 'chrome' | undefined {
  ...
  try {
    ...
    if (execPath && existsSync(execPath)) {
      brLog.info('chromium_bundled', { execPath });
      _cachedChannel = undefined;
      return _cachedChannel;
    }
  } catch { /* executablePath() may throw if browser is not installed */ }

  brLog.info('chromium_system_fallback');
...
export function fixCrashedProfile(profileDir: string): void {
  ...
  try {
    ...
    if (prefs.profile?.exit_type !== 'Normal' || !prefs.profile?.exited_cleanly) {
      ...
      brLog.info('fixed_crashed_profile', { profileName: path.basename(profileDir) });
    }
...
export function killStaleChrome(profileDir: string): void {
  if (!/^[\w\s/\\:.\-_~]+$/.test(profileDir)) {
    brLog.warn('killStaleChrome_skipped_unsafe_path', { profileDir });
    return;
  }
  try {
    let pids: string[] = [];
    ...
    if (process.platform === 'win32') {
      // `wmic` is invoked via argv; filter expressions are interpolated INTO a
      // query string but never into a shell. We still constrain profileDir to
      // safe characters above.
      const escapedDir = profileDir.replace(/\\/g, '\\\\');
      const res = spawnSync('wmic', [
        'process',
        'where', `name='chrome.exe' and commandline like '%${escapedDir}%'`,
        'get', 'processid', '/value',
      ], { encoding: 'utf8', timeout: 5000, shell: false });
      const stdout = res.stdout ?? '';
      pids = (stdout.match(/ProcessId=(\d+)/g) ?? [])
        .map((m) => m.split('=')[1])
        .filter((pid): pid is string => !!pid && /^\d+$/.test(pid));
    } else {
      // Unix: run `ps aux` and parse output in JS — no shell pipeline.
      const res = spawnSync('ps', ['aux'], { encoding: 'utf8', timeout: 5000, shell: false });
      const stdout = res.stdout ?? '';
      pids = stdout.split('\n')
    ...
    if (unique.length) {
      brLog.info('killed_stale_chrome', {
...
export async function launchPersistentContextWithRetry(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      brLog.warn('launch_context_retry', { attempt, nextDelaySec: 3 * attempt, message: msg });
      await delay(3000 * attempt);
    }
  }
