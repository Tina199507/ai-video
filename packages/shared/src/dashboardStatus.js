...
export function getDashboardStatus(project) {
    const ss = project.stageStatus ?? {};
    if (project.error || Object.values(ss).some((s) => s === 'error'))
        return 'error';
    if (ALL_STAGES.every((s) => ss[s] === 'completed'))
        return 'completed';
    ...
    if (ss.QA_REVIEW !== 'completed')
        return 'analysis';
    if (ss.KEYFRAME_GEN !== 'completed')
        return 'visualGenerating';
    return 'assembling';
}
export function getCardAction(_project, status) {
    if (status === 'scriptReview')
        return { label: '审核脚本', target: 'script' };
    if (status === 'visualReview' || status === 'visualGenerating')
        return { label: '审核视觉', target: 'storyboard' };
    if (status === 'completed')
        return { label: '查看成片', target: 'production' };
    if (status === 'error')
        return { label: '查看错误', target: 'production' };
    if (status === 'assembling')
        return { label: '查看进度', target: 'production' };
